...
function SpawnWorkbenches()
    for i, wb in ipairs(Config.Workbenches) do
        ...
        RequestCollisionAtCoord(wb.coords.x, wb.coords.y, wb.coords.z)
        local prop = CreateObjectNoOffset(
            modelHash, wb.coords.x, wb.coords.y, wb.coords.z + 1.0, false, false, false
        )
        ...
        local grounded = WorkbenchPlacement.Place(prop, wb)
        if Config.Debug and not grounded then
            print(('[COREX-CRAFTING] ^3Ground collision unavailable; used configured Z for workbench %d^0'):format(i))
        end
        SetModelAsNoLongerNeeded(modelHash)
        ...
        local success, result = pcall(function()
            return exports.ox_target:addLocalEntity(prop, {
                {
                    name = 'corex_crafting_workbench_' .. i,
                    icon = wb.icon or 'fa-solid fa-hammer',
                    label = wb.interactLabel or (wb.label or 'Workbench'),
                    distance = wb.radius or 2.5,
                    onSelect = function()
                        TriggerEvent('corex-crafting:client:openAtWorkbench', prop, { workbenchIndex = i })
                    end
                }
            })
        end)
...
RegisterNetEvent('corex-crafting:client:craftComplete', function(recipeId)
    ...
    lib.notify({ description = 'Item ready! Open crafting to collect.', type = 'success', duration = 4000 })
end)
