...
function renderRecipeList(category, preserveScroll = false) {
    ...
    filtered.forEach((recipe, index) => {
        ...
        if (recipe.locked) {
            classes += ' locked';
            if (recipe.skillLocked) classes += ' skill-locked';
        }
        ...
        // Specific lock reason wins the badge — generic "Locked" hides why.
        let statusText;
        let badgeClass = '';
        if (recipe.skillLocked) {
            statusText = 'Skill';
            badgeClass = ' skill-locked';
        } else if (recipe.locked) {
            statusText = 'Blueprint';
            badgeClass = ' blueprint-locked';
        } else if (craftable) {
            statusText = 'Ready';
            badgeClass = ' craftable';
        } else {
            statusText = 'Short';
        }
        ...
        card.innerHTML = `
            <div class="card-main-content">
                ...
                <div class="card-status-badge${badgeClass}">${statusText}</div>
            </div>
            <div class="card-mats">
                ${materialDots}
            </div>
        ...
        // Always allow click — even on locked cards — so the player can read
        // the detail panel and understand WHY (skill / blueprint) it's locked.
        {
...
function renderRecipeDetails(recipeId) {
    ...
    // Skill requirement banner (only visible when the recipe has a gate).
    const skillReqEl = document.getElementById('skill-req');
    if (recipe.skillRequirement && recipe.skillRequirement.name) {
        skillReqEl.classList.remove('hidden');
        skillReqEl.classList.toggle('met', recipe.skillRequirement.met === true);
        document.getElementById('skill-req-name').textContent = recipe.skillRequirement.name;
        document.getElementById('skill-req-status').textContent =
            recipe.skillRequirement.met ? 'Unlocked' : 'Locked';
    } else {
        skillReqEl.classList.add('hidden');
    }
