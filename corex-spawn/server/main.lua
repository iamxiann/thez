...
function CheckAndSpawnPlayer(source, player, options)
    ...
    if Player(source).state.characterSelection then
        Debug('Info', 'Spawn blocked while character selection is active: ' .. source)
        return
    end
    ...
    -- Resend only the server-approved pending spawn; active players cannot respawn here.
    if spawnedPlayers[source] and not isResourceRestart and not isRespawn then
        local pending = spawnedPlayers[source]
        if lifecycleState == 'loading' and GetGameTimer() - pending.sentAt >= 5000 then
            pending.sentAt = GetGameTimer()
            TriggerClientEvent('corex-spawn:client:spawnPlayer', source, pending.data)
        end
        return
    end
    ...
    local skin = exports['illenium-appearance']:GetAppearance(source)
    -- Exports return table copies, so compare stable identity after the database await.
    local currentPlayer = GetPlayer(source)
    if not currentPlayer or not player.identifier or currentPlayer.identifier ~= player.identifier then return end
    ...
    if skin then
        ...
        spawnData = {
            ...
            playerData = { name = name, money = money, metadata = player.metadata }
        }
    end

    spawnSequence = spawnSequence + 1
    spawnData.spawnId = spawnSequence
    spawnedPlayers[source] = { data = spawnData, sentAt = GetGameTimer() }
    TriggerClientEvent('corex-spawn:client:spawnPlayer', source, spawnData)
...
RegisterNetEvent('corex-spawn:server:checkPlayer', function()
    local source = source
    if Player(source).state.characterSelection then return end
...
-- Only corex-death's validated server flow may authorize a respawn.
AddEventHandler('corex-spawn:server:respawnPlayer', function(source)
    local player = GetPlayer(source)
    if not player then return end
    local state = GetSpawnState(source)
    if state ~= 'dead' and state ~= 'loading' then return end
...
RegisterNetEvent('corex-spawn:server:markSpawnReady', function(position, spawnId)
    local source = source
    local player = GetPlayer(source)
    if not player then return end
    local pending = spawnedPlayers[source]
    if not pending or spawnId ~= pending.data.spawnId then return end
    local state = GetSpawnState(source)
    if state == 'active' and readyForPositionSave[source] then
        TriggerClientEvent('corex-spawn:client:spawnConfirmed', source, spawnId)
        return
    end
    if state ~= 'loading' then return end
    if pending.readyAt and GetGameTimer() - pending.readyAt < 1000 then return end
    pending.readyAt = GetGameTimer()
    if not exports['illenium-appearance']:GetAppearance(source) then
    ...
    if not currentPlayer or not player.identifier or currentPlayer.identifier ~= player.identifier then return end
    player = currentPlayer
    if spawnedPlayers[source] ~= pending or GetSpawnState(source) ~= 'loading' then return end
    SetSpawnState(source, 'active')
    if GetSpawnState(source) ~= 'active' then return end
    readyForPositionSave[source] = true
    TriggerClientEvent('corex-spawn:client:spawnConfirmed', source, spawnId)

    if Config.SaveLastLocation and IsValidSpawnPosition(position) then
        pcall(function()
...
RegisterCommand('resetchar', function(source, args, rawCommand)
    ...
    if not exports['illenium-appearance']:DeleteAppearance(source) then return end
    pcall(function()
    ...
    TriggerClientEvent('corex-spawn:client:clearSpawnFlags', source)
    CheckAndSpawnPlayer(source, GetPlayer(source), { isRespawn = true })
