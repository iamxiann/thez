...
RegisterCommand('spawnzombie', function(source, args, rawCommand)
    if source ~= 0 then
        if not Corex.Functions.IsPlayerAceAllowed(source, 'corex.admin') then
            TriggerClientEvent('ox_lib:notify', source, { description = 'No permission', type = 'error', duration = 3000 })
            return
        end
    end
    ...
    if source == 0 then
        ...
        TriggerClientEvent('ox_lib:notify', source, { description = 'Spawning ' .. count .. ' ' .. (typeId or 'random') .. ' zombies', type = 'success', duration = 3000 })
    end
end, true)
