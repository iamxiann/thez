...
local function GetSafeZoneDistance(coords)
    if not coords or GetResourceState('corex-zones') ~= 'started' then
        return math.huge
    end

    local success, distance = pcall(function()
        return exports['corex-zones']:GetSafeZoneDistance(coords)
    end)
    if success and type(distance) == 'number' and distance >= 0.0 then
        return distance
    end
    return math.huge
end
...
function IsNearSafeZone(coords)
    -- Safe zones expose polygon distance; their config has no circle center/radius.
    return GetSafeZoneDistance(coords) <= (Config.SafeZoneBuffer or 20.0)
end
...
local function ResolveDynamicBudget(playerCoords)
    ...
    local dist = GetSafeZoneDistance(playerCoords)
    if dist == math.huge then dist = 99999.0 end
...
local function StartGrab(zombieEntity)
    ...
    -- Escape mechanic: repeated ox_lib skill checks. Each success adds to
    -- escapeProgress; failing one just costs a short cooldown (and more
    -- grab damage from ApplyGrabDamage's own loop) before the next try.
    local mySession = grabSession
    CreateThread(function()
        while isGrabbed and grabSession == mySession do
            local grabConfig = GetGrabConfig()
            local success = lib.skillCheck(grabConfig.skillCheckDifficulty or 'medium')

            if not isGrabbed or grabSession ~= mySession then break end

            if success then
                escapeProgress = escapeProgress + 1
                if escapeProgress >= (grabConfig.escapeRequired or 10) then
                    EndGrab(true)
                    break
                end
            else
                Wait(grabConfig.escapeCooldown or 400)
            end
        end
    end)
end

EndGrab = function(escaped)
...
CreateThread(function()
    ...
    while true do
        ...
        if #activeZombies == 0 then goto runnerContinue end
...
CreateThread(function()
    ...
    while true do
        ...
        if #activeZombies == 0 then
            -- Keep the toxic fade cadence without querying players in an empty world.
            ProcessToxicAura(nil, nil, GetGameTimer())
            goto aiContinue
        end
        ...
        for _, zombieData in ipairs(activeZombies) do
            if not zombieData.isDead and now >= (zombieData.nextAiAt or 0) and DoesEntityExist(zombieData.entity) then
                ...
                playerCache = playerCache or BuildPlayerCache(playerPed)
...
CreateThread(function()
    ...
    while true do
        Wait(1000)
        if #activeZombies == 0 then goto cleanupContinue end
...
CreateThread(function()
    ...
    while true do
        ...
        if #activeZombies == 0 then goto continue end
        local playerCoords = GetEntityCoords(PlayerPedId())
        for _, zombieData in ipairs(activeZombies) do
            if DoesEntityExist(zombieData.entity) and not IsPedDeadOrDying(zombieData.entity, true) then
                local zombieCoords = GetEntityCoords(zombieData.entity)
