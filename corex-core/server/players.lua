...
function COREX.Player.Create(source, dbData, isNewPlayer)
    ...
    local identifier = dbData.identifier or GetIdentifier(source, 'steam')
    ...
    local newPlayer = {
        ...
        accountIdentifier = GetIdentifier(source, 'steam'),
...
function COREX.Player.Load(source)
    if Config.Multicharacter then
        TriggerEvent('corex-multicharacter:server:requestSelection', source)
        return
    end
...
-- Server-only entry point. The account/character relationship is checked again here.
exports('LoadCharacter', function(src, identifier)
    if GetInvokingResource() ~= 'corex-multicharacter' or not Config.Multicharacter then return false end
    if type(src) ~= 'number' or type(identifier) ~= 'string' or #identifier > 60 then return false end
    if Players[src] or loadingPlayers[src] then return false end
    local account = GetIdentifier(src, 'steam')
    if not account then return false end
    for other, player in pairs(Players) do
        if other ~= src and player.accountIdentifier == account then return false end
    end
    local token = {}
    loadingPlayers[src] = token
    local ok, row = pcall(MySQL.single.await, [[
        SELECT p.* FROM players p INNER JOIN corex_characters c ON c.identifier = p.identifier
        WHERE c.account = ? AND c.identifier = ? AND c.slot IS NOT NULL AND c.deleted_at IS NULL LIMIT 1
    ]], { account, identifier })
    if loadingPlayers[src] ~= token then return false end
    loadingPlayers[src] = nil
    if not ok or not row or GetIdentifier(src, 'steam') ~= account then return false end
    return COREX.Player.Create(src, row, false) ~= nil
end)
...
AddEventHandler('playerConnecting', function(playerName, setKickReason, deferrals)
    ...
    if not identifier then
        setKickReason('Steam identifier tidak ditemukan. Buka Steam lalu sambungkan kembali.')
        CancelEvent()
        return
    end
