...
function COREX.Server.GetNearbyPlayers(source, maxDistance)
    ...
    local origin = COREX.Server.GetPlayerCoords(source)
    if not origin then return nearby end

    for _, otherSource in ipairs(COREX.Server.GetPlayerSources()) do
        if otherSource ~= source then
            local coords = COREX.Server.GetPlayerCoords(otherSource)
            local distance = coords and COREX.Utils.GetDistanceVec(origin, coords)
