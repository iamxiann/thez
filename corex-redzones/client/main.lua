...
local function CleanupCrateEntry(entry)
    ...
    if entry.prop and DoesEntityExist(entry.prop) then
        pcall(function() exports.ox_target:removeLocalEntity(entry.prop) end)
        if Corex then
            ...
            if prop then
                ...
                local ok = pcall(function()
                    exports.ox_target:addLocalEntity(prop, {
                        {
                            name = 'corex_redzones_crate_' .. crate.crateId,
                            icon = 'fa-solid fa-briefcase-medical',
                            label = 'Search Military Crate',
                            distance = Config.CrateInteractDistance,
                            onSelect = function()
                                TriggerEvent('corex-redzones:client:searchCrate', prop, { crateId = crate.crateId })
                            end
                        }
                    })
                end)
            ...
            if box then
                ...
                pcall(function()
                    exports.ox_target:addLocalEntity(box, {
                        {
                            name = 'corex_redzones_bigbox_' .. boxId,
                            icon = 'fa-solid fa-box-open',
                            label = 'Open Big Loot Box',
                            distance = Config.CrateInteractDistance + 0.5,
                            onSelect = function()
                                TriggerEvent('corex-redzones:client:searchCrate', box, { crateId = boxId })
                            end
                        }
                    })
                end)
...
local function EnterZone(zone)
    ...
    lib.notify({ description = 'Entered Red Zone: ' .. zone.name, type = 'error', duration = 5000 })
...
local function LeaveZone()
    ...
    lib.notify({ description = 'Left Red Zone', type = 'warning', duration = 3000 })
