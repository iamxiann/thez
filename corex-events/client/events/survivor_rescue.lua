...
RegisterNetEvent('corex-events:client:survivorSpawn', function(eventId, data)
    ...
    lib.notify({ description = 'Survivors found! Protect them from incoming waves!', type = 'warning', duration = 6000 })
end)
...
RegisterNetEvent('corex-events:client:survivorWave', function(eventId, data)
    ...
    lib.notify({ description = ('Wave %d/%d incoming!'):format(data.wave, Config.SurvivorRescue.zombieWaves), type = 'error', duration = 4000 })
end)
