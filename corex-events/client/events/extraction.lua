...
local function ClearEntities()
    for _, e in ipairs(spawnedEntities) do
        ...
        elseif DoesEntityExist(e) then
            pcall(function() exports.ox_target:removeLocalEntity(e) end)
            DeleteEntity(e)
        end
    end
    spawnedEntities = {}
...
function handler.update(state, patch)
    ...
    lib.notify({ description = 'Extraction point active! Defend the zone until the helicopter arrives!', type = 'error', duration = 7000 })
    ...
    CreateThread(function()
        ...
        for i = holdSec, 1, -1 do
            ...
            if i <= 10 or i % 30 == 0 then
                ...
                lib.notify({ description = ('Extraction in %s - hold the zone!'):format(timeStr), type = 'warning', duration = 3000 })
            end
            Wait(1000)
        end
    end)
...
RegisterNetEvent('corex-events:client:extractionReward', function(rewardId, data)
    ...
    local Corex = CXEC_GetCorex()
    if not Corex then return end
    lib.notify({ description = 'Extraction complete! Helicopter is inbound!', type = 'success', duration = 5000 })
end)
