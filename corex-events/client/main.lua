...
do
        ...
        if target.event then
            local ok, err = pcall(function()
                exports.ox_target:addLocalEntity(prop, {
                    {
                        name = 'corex_events_crate_' .. tostring(options.eventId or GetGameTimer()),
                        icon = target.icon or 'fa-solid fa-box-open',
                        label = target.text or 'Open Crate',
                        distance = target.distance or 2.5,
                        onSelect = function()
                            TriggerEvent(target.event, prop, target.data or {})
                        end
                    }
                })
            end)
    ...
    function CXEC_CleanupRewardCrate(ctx)
        ...
        if ctx.prop then
            pcall(function()
                exports.ox_target:removeLocalEntity(ctx.prop)
            end)
