...
AddEventHandler("onResourceStart", function(resource)
    if resource == GetCurrentResourceName() then
        if Framework.Corex() and not LocalPlayer.state.isLoggedIn then return end
        InitAppearance()
    end
end)
...
function InitializeCharacter(gender, onSubmit, onCancel)
    ...
    local opened = client.startPlayerCustomization(function(appearance)
        if (appearance) then
            if not Framework.FinishCharacterCreation or not Framework.FinishCharacterCreation() then
