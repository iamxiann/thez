...
function IsPlayerAllowedForOutfitRoom(outfitRoom)
    ...
    local count = #(outfitRoom.identifiers or {})
    for i = 1, count, 1 do
        if Framework.IsPlayerAllowed(outfitRoom.identifiers[i]) then
            isAllowed = true
            break
        end
    end
    return isAllowed or not outfitRoom.identifiers or count == 0
end
...
function OpenSurgeonShop(hasTicketAccess)
    if not hasTicketAccess then
        lib.notify({
            title = _L("menu.surgeonShopTitle"),
            description = "Gunakan  Tiket Oplas di lokasi surgeon untuk membuka menu.",
            type = "error",
