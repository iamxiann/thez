...
local function MigrateFivemAppearance(source)
    ...
    for i=1, #allPlayers, 1 do
        if allPlayers[i].skin then
            playerSkins[#playerSkins+1] = {
                identifier = allPlayers[i].identifier,
    ...
    for i=1, #playerSkins, 1 do
        Database.PlayerSkins.Add(playerSkins[i].identifier, json.decode(playerSkins[i].skin).model, playerSkins[i].skin, 1)
    end
    lib.notify(source, {
...
RegisterNetEvent("illenium-appearance:server:migrate-qb-clothing-skin", function(identifier, appearance)
    local src = source
