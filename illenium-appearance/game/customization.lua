...
local function allowedForPlayer(item, allowedAces)
    return (item.jobs and listContains(item.jobs, client.job.name)) or (item.gangs and listContains(item.gangs, client.gang.name)) or (item.aces and listContainsAny(item.aces, allowedAces) or (item.identifiers and listContains(item.identifiers, client.identifier)))
end
...
local function filterPedModelsForPlayer(pedConfigs)
    ...
    for i = 1, #pedConfigs do
        ...
        if (not config.jobs and not config.gangs and not config.aces and not config.identifiers) or allowedForPlayer(config, allowedAces) then
...
local function getHeadBlendShapeMax(ped)
    local maxShape = 45
    local modelName = getFreemodeModelName(ped)

    if not modelName or not Config.HeadBlend then
        return maxShape
    end

    local function checkHeadBlend(headBlend)
        if type(headBlend) ~= "table" then return end

        maxShape = math.max(maxShape, tonumber(headBlend.shapeFirst) or maxShape)
        maxShape = math.max(maxShape, tonumber(headBlend.shapeSecond) or maxShape)
        maxShape = math.max(maxShape, tonumber(headBlend.shapeThird) or maxShape)
    end
    ...
    for modelKey, modelHeads in pairs(Config.HeadBlend) do
        ...
        if configuredModelName == modelName and type(modelHeads) == "table" then
            for _, headConfig in pairs(modelHeads) do
                if type(headConfig) == "table" then
                    checkHeadBlend(headConfig.headBlend)
                end
            end
        end
    end
...
local function getAppearanceSettings()
    ...
    local headBlendShapeMax = getHeadBlendShapeMax(cache.ped)
    local headBlend = {
        shapeFirst = {
            min = 0,
            max = headBlendShapeMax
        },
        shapeSecond = {
            min = 0,
            max = headBlendShapeMax
...
function client.startPlayerCustomization(cb, conf)
    ...
    while not IsScreenFadedIn() or IsPlayerTeleportActive() or IsPlayerSwitchInProgress() do
        if GetGameTimer() - startedAt >= 15000 then return false end
        Wait(50)
    end
