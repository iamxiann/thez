...
-- Independent server-side point-in-polygon check via the shared PolyZone
-- objects (built once in shared/zones.lua, same objects/config as client).
-- This is what keeps the anti-cheat property: the server never trusts the
-- client's own enter/leave report, it always re-derives zone membership
-- itself from the player's actual server-known position.
local function GetSafeZoneForCoords(coords)
    local _, zoneConfig = Corex_GetZoneObjectAt(coords)
    return zoneConfig
end
