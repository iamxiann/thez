        ...
        if shopData.npc then
            ...
            if npc then
                ...
                local success, result = pcall(function()
                    return exports.ox_target:addLocalEntity(npc, {
                        {
                            name = 'corex_inventory_shop_' .. shopName,
                            icon = shopData.npc.icon or 'fa-solid fa-store',
                            label = shopData.npc.interactLabel or shopData.label,
                            distance = shopData.npc.interactDistance or 2.5,
                            onSelect = function()
                                TriggerEvent('corex-inventory:client:openShopFromInteract', npc, { shopName = shopName })
                            end
                        }
                    })
                end)
...
AddEventHandler('corex-inventory:client:openShopFromNPC', function(shopName)
    ...
    if Shops[shopName] then
        ...
        lib.notify({ description = 'Shop not available!', type = 'error', duration = 3000 })
    end
end)
